...
🔥 WITH STARTING ANIMATION
📸 ALL BUTTONS FIXED - RAILWAY READY
"""
...
def get_remaining(expiry_str):
    try:
        ...
        if days > 30:
            return f"{days//30}M+", False
        elif days > 0:
            return f"{days}D {hours}H {minutes}M", False
        elif hours > 0:
            return f"{hours}H {minutes}M {seconds}S", False
        elif minutes > 0:
            return f"{minutes}M {seconds}S", False
        else:
...
# ═══════════════ USER FUNCTIONS ═══════════════
def get_users():
    return jload(USERS_DB, {"premium": [], "keys": {}})
...
def get_user_info(uid):
    ...
    info = {"access": access, "type": a_type, "threads": PREMIUM_THREADS if access else 0,
            "max_time": PREMIUM_TIME if access else 0, "expiry": None, "remaining": None}
...
def grant_access(uid, key_name, duration_str, expiry):
    ...
    if "keys" not in u:
        u["keys"] = {}
    u["keys"][str(uid)] = {"key_name": key_name, "duration": duration_str,
                           "expiry": expiry.isoformat(), "granted_at": datetime.now(IST).isoformat()}
    jsave(USERS_DB, u)
...
def create_key(name, time_str):
    ...
    if not parsed:
        return None, None
    value, unit = parsed
    keys[key_code] = {"name": name, "time_value": value, "time_unit": unit,
                      "duration_display": format_duration(value, unit),
                      "created": datetime.now(IST).isoformat(), "used_by": None, "active": True}
    jsave(KEYS_DB, keys)
...
class Attack:
    ...
    def start(self, ip, port, dur, threads):
        ...
        self.on = False
        e = max(dur, 0.1)
        return {'pkts': self.pkts, 'mbps': (self.bytes_out * 8) / (e * 1e6),
